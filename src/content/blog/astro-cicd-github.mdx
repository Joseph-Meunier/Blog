...
- Un site Astro fonctionnel et uniquement statique (si non , suis mon tutoriel [Créer un blog moderne avec Astro](/Blog/blog/astro-blog/)),
...
export default defineConfig({
  site: 'https://monuser.github.io/nom-du-repo',
  base: '/nom-du-repo', 
});
```
...
export default defineConfig({
  site: 'https://monuser.github.io/Blog',
  base: isProd ? '/Blog' : '/',
});
```
    ...
    steps:
      ...
      - name: Deploy
        ...
        with:
          ...
          enable_jekyll: false
```
Ce document définit un workflow qui s’exécutera à chaque push sur la branche master quand tu auras configuré GitHub Pages pour publier la branche gh-pages. (Voir étape suivante.).
...
Dans Build and Deployment → Source, choisis Deploy from a branch, puis sélectionne la branche gh-pages.
